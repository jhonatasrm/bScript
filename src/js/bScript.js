var style, _style, enabled = false, prefix = 'ujs_bScript', none = '{display: none !important;}';
...
var getNth = function(element){
	var nth, n = 0, p = element.parentNode;
   	for(var i = 0, c; c = p.childNodes[i]; i++){if(c.nodeType == 1){n++; if(c == element)nth = n}};
	...
	while(element){
		if(element.nodeType == 1){
			...
			if(att){
				...
				rez.unshift(tag+getAtt(element, 'id|class|height|width|color|bgcolor|align|type')+((wide || /^(html|body)$/i.test(tag)) ? '' : getNth(element)));
			}
		}
...
if(enabled)return;
var element = '', outline = '', border = '', bgColor = '', title = '', reObjects = /^(iframe|object|embed)$/i;
...
var click = function(event){
	if(element){
		...
		var tmpCss = addStyle(css+'{background-color: red !important; border: 1px solid #FF1111 !important; opacity: 0.7 !important;}');
...
document.addEventListener('keydown', function(e){
	if(!e.shiftKey && !e.ctrlKey && e.altKey){
	    switch(e.keyCode){
	        case 85: unblockElement(); break;
			case 66: blockElement(); break;
			case 76: unblockElement(true); break;
		}
	}
}, false);