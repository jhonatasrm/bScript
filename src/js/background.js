...
browser.webRequest.onHeadersReceived.addListener(add_csp_nojs_header,{urls: ["<all_urls>"], types: ["main_frame"]},["blocking", "responseHeaders"]);
...
browser.tabs.onUpdated.addListener((id, changeInfo) => {
    if (changeInfo.url) {
	    ...
	    browser.storage.local.get(host).then(item => {
	        ...
	        let status_title = whitelist_js ? browser.i18n.getMessage("javascriptEnabled") : browser.i18n.getMessage("javascriptDisabled");
